...
Προαιρετικό EXE:
  pyinstaller --onefile --windowed --uac-admin --name StorageAnalyzer --icon app.ico storage_analyzer.py
"""
...
class StorageAnalyzerApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.summary_tree.heading("folders", text="Φάκελοι")
        ...
        self.summary_tree.column("folders", width=90, anchor="e")
        self.summary_tree.column("errors", width=90, anchor="e")
        self.summary_tree.column("skipped", width=145, anchor="e")
        self.summary_tree.column("path", width=590, anchor="w")
        ...
        self.files_tree.heading("size", text="Μέγεθος")
        self.files_tree.heading("bytes", text="Bytes")
        self.files_tree.heading("modified", text="Τροποποίηση")
        self.files_tree.heading("path", text="Αρχείο")
        self.files_tree.column("size", width=120, anchor="e")
        self.files_tree.column("bytes", width=130, anchor="e")
        self.files_tree.column("modified", width=170, anchor="center")
        self.files_tree.column("path", width=760, anchor="w")
    ...
    def _on_user_selected(self, _event=None) -> None:
        ...
        result = next((r for r in self.results if r.user_name == user_name), None)
        if not result:
            return
        ...
        for index, lf in enumerate(result.large_files):
            modified = (
                datetime.fromtimestamp(lf.modified_ts).strftime("%Y-%m-%d %H:%M:%S")
                if lf.modified_ts
                else ""
